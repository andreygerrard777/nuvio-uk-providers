// Nuvio provider: KinoTron (kinotron.tv)
// Hand-ported from the CloudStream Kotlin provider KinoTronProvider.kt
// (https://github.com/CakesTwix/cloudstream-extensions-uk).
//
// Fully confirmed against real, live-fetched pages: search results
// (.th-item / .th-in / .th-title), the movie player chain
// (div.video-box iframe[data-src] -> ashdi.vip/vod/<id> -> single file:'...m3u8')
// and the series player chain (ashdi.vip/serial/<id> -> file:'[JSON dubs]').
// Some pages list a YouTube trailer as an extra .video-box before or after the
// real player; the first non-YouTube one is used (the same duplicate-player
// quirk seen on other sites in this family).
//
// Nuvio only gives us a TMDB id, not a title, so this provider resolves the
// TMDB id to a title via the TMDB API, then runs it through KinoTron's own
...
// --- Site search -------------------------------------------------------
// Confirmed live: <div class="th-item"><a class="th-in" href="..."><div class="th-title">Title</div>...
...
function parseSearchResults(html) {
  ...
  while ((m = re.exec(html))) {
    var attrs = m[1];
    if (!/class="[^"]*\bth-in\b[^"]*"/.test(attrs)) continue;
    var hrefMatch = /href="([^"]+)"/.exec(attrs);
    if (!hrefMatch) continue;
    var titleMatch = /class="th-title[^"]*"[^>]*>([^<]*)</.exec(m[2]);
    var title = (titleMatch ? titleMatch[1] : stripTags(m[2])).trim();
    if (title) results.push({ href: hrefMatch[1], title: title });
  }
  return results;
}

function searchKinoTron(query) {
  var body = 'do=search&subaction=search&story=' + encodeURIComponent(query).replace(/%20/g, '+');
  return fetchHtml(BASE + '/index.php?do=search', {
    method: 'POST',
...
// --- Player resolution -------------------------------------------------
// ashdi.vip/vod/<id> pages return a single file:'...m3u8'; ashdi.vip/serial/<id>
// pages return file:'[JSON array of dubs -> seasons -> episodes]' - the two
// content types live at different URL paths here (no query-param toggle,
// unlike the sibling Uakino/KlonTV sites).

function extractPlaynd(html) {
  var re = /class="[^"]*video-box[^"]*"[\s\S]{0,400}?<iframe[^>]+data-src="([^"]+)"/g;
  var m;
  while ((m = re.exec(html))) {
    if (!/youtube\.com|youtu\.be/i.test(m[1])) return m[1];
  }
  return null;
}
...
function resolveMovieStream(playerUrl) {
  return fetchHtml(playerUrl).then(function (html) {
    ...
    return [{
      name: 'KinoTron',
      title: 'KinoTron ' + guessQuality(m[1]),
      url: m[1],
...
function resolveTvStream(playerUrl, season, episode) {
  return fetchHtml(playerUrl).then(function (html) {
    var m = /file\s*:\s*'(\[[\s\S]*?\])'/.exec(extractScripts(html));
    if (!m) return [];
    var dubs;
...
function getStreams(tmdbId, mediaType, season, episode) {
  return getTmdbInfo(tmdbId, mediaType).then(function (info) {
    ...
    return searchKinoTron(info.title).then(function (results) {
      if (results.length || !info.originalTitle || info.originalTitle === info.title) return results;
      return searchKinoTron(info.originalTitle);
